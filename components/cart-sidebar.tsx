...
export function CartSidebar() {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    const phoneNumber = "5493816464923"
  ...
  return (
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent className="w-full sm:max-w-md bg-graphite border-l-white/10 flex flex-col p-0">
        <div className="p-6 border-b border-white/10">
          <SheetHeader className="flex flex-row items-center justify-between">
            <SheetTitle className="text-2xl font-black text-foreground">TU SELECCIÓN</SheetTitle>
          </SheetHeader>
        </div>
          ...
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
              ...
              <button
                ...
                className="mt-6 text-celeste font-bold tracking-[0.1em] hover:text-white transition-colors"
              >
                SEGUIR EXPLORANDO
              </button>
            </div>
            ...
            <div className="space-y-6">
              {items.map((item) => (
                <div key={item.id} className="flex gap-4 p-4 bg-background/50 rounded-xl border border-white/5">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h4 className="font-bold text-foreground line-clamp-2 leading-tight">{item.name}</h4>
                      <p className="text-[10px] text-muted-foreground mt-1 uppercase tracking-wider">Talles: {item.sizes}</p>
                    </div>
